    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {advantages.map((adv, index) => (
                <div key={index} className="p-6 text-center shadow-lg rounded-2xl border border-gray-200 bg-white">
                    <div className="flex justify-center mb-4">{adv.icon}</div>
                    <h3 className="text-xl font-semibold">{adv.title}</h3>
                    {adv.description && <p className="text-gray-600 mt-2">{adv.description}</p>}
                </div>
            ))}
        </div>
    );
